...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function loginWithGoogle() {
    ...
    try {
      logger.debug('Attempting Google login', { domain: window.location.hostname }, 'Auth')
      
      const result = await signInWithPopup(auth, provider)
      const user = result.user
      
      logger.authSuccess('Google login successful', { email: user.email })
      ...
      logger.authError('Google login failed', { 
        code: error.code, 
